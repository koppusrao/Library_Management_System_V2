...
function OverdueReturnReport() {
  ...
  const loadData = () => {
    axios
      ...
      .then((res) => {
        // FIX: Our gateway returns an ARRAY, not { loans: [...] }
        const allLoans = Array.isArray(res.data) ? res.data : res.data.loans || [];

        const today = new Date();

        const overdue = allLoans.filter((loan) => {
          if (loan.status !== "borrowed") return false; // only active loans matter

          const due = protoToJsDate(loan.due_date);
          if (!due) return false;

          return due < today; // overdue condition
        });

        // Optional: sort by due date ascending
        overdue.sort((a, b) => {
          const da = protoToJsDate(a.due_date);
          const db = protoToJsDate(b.due_date);
          return da - db;
        });
          ...
          <tbody>
            {loans.map((loan) => (
              <tr key={loan.id}>
                ...
                <td>
                  {loan.due_date
                    ? `${loan.due_date.year}-${String(loan.due_date.month).padStart(
                        2,
                        "0"
                      )}-${String(loan.due_date.day).padStart(2, "0")}`
                    : ""}
                </td>
              </tr>
            ))}
